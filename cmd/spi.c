...
static unsigned int	sspi_rate = 1000000;
...
static int do_spi_xfer(int bus, int cs)
{
	...
	if (!str)
		return -ENOMEM;
	ret = spi_get_bus_and_cs(bus, cs, sspi_rate, mode, "spi_generic_drv",
				 str, &dev, &slave);
	if (ret)
		return ret;
#else
	slave = spi_setup_slave(bus, cs, sspi_rate, mode);
	if (!slave) {
...
int do_spi_rate(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	char  *cp = 0;

	sspi_rate = simple_strtoul(argv[1], &cp, 10);
	printf("sspi rate is %d\n", sspi_rate);
	return 0;
}
