...
libs-$(CONFIG_SYS_CTC_DDR) += drivers/ddr/centec/
...
ALL-y += u-boot.dat u-boot.srec u-boot.bin u-boot.sym System.map binary_size_check
...
ALL-$(CONFIG_SPL) += spl/u-boot-spl.bin spl/u-boot-spl.dat spl/ctc5236-spl.img
...
MKIMAGEFLAGS_u-boot.img = -A $(ARCH) -T ctc5236image -C none -O u-boot \
	-a $(CONFIG_SYS_TEXT_BASE) -e $(CONFIG_SYS_UBOOT_START) \
...
MKIMAGEFLAGS_ctc5236-spl.img = -A $(ARCH) -T ctc5236splimage -C none \
	-a $(CONFIG_SPL_TEXT_BASE) -e $(CONFIG_SPL_TEXT_BASE) -n CTC5236

spl/ctc5236-spl.img: spl/u-boot-spl.bin FORCE
	$(call if_changed,mkimage)

OBJCOPYFLAGS_ctc5236-spl.bin = -I binary -O binary --pad-to=$(CONFIG_SPL_PAD_TO)
OBJCOPYFLAGS_ctc5236-qspi-spl.bin = -I binary -O binary --pad-to=$(CONFIG_SPL_PAD_TO)

spl/ctc5236-spl.bin: spl/ctc5236-spl.img FORCE
	$(call if_changed,objcopy)

spl/ctc5236-qspi-spl.bin: spl/u-boot-spl.bin FORCE
	$(call if_changed,objcopy)

ctc5236-full.bin: spl/ctc5236-spl.bin u-boot.img FORCE
	$(call if_changed,cat)

ctc5236-qspi-full.bin: spl/ctc5236-qspi-spl.bin u-boot.img FORCE
	$(call if_changed,cat)
...
quiet_cmd_dat ?= DAT     $@
	cmd_dat ?= $(objtree)/tools/bin2dat.pl u-boot.bin u-boot.dat
u-boot.dat: u-boot.bin FORCE
	$(call if_changed,dat)

quiet_cmd_spldat ?= SPL_DAT $@
	cmd_spldat ?= $(objtree)/tools/bin2dat.pl spl/u-boot-spl.bin spl/u-boot-spl.dat
spl/u-boot-spl.dat: spl/u-boot-spl.bin FORCE
	$(call if_changed,spldat)
