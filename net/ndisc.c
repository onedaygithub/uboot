...
static void ip6_send_ns(struct in6_addr *neigh_addr)
{
	...
	/* checksum */
	msg->icmph.icmp6_cksum = htons(csum_ipv6_magic(&net_link_local_ip6, &dst_adr,
 ...
 struct in6_addr *target)
{
	...
	/* checksum */
	msg->icmph.icmp6_cksum = htons(csum_ipv6_magic(&net_link_local_ip6,
...
{
	debug("ndisc_request %pI6c  %pI6c\n", &net_ip6, &net_nd_sol_packet_ip6);
...
{
	struct icmp6hdr *icmp = (struct icmp6hdr *)(((uchar *)ip6) + IP6_HDR_SIZE);
	...
	switch (icmp->icmp6_type) {
		case IPV6_NDISC_NEIGHBOUR_SOLICITATION:
			...
			if (ip6_is_our_addr(&ndisc->target) && ndisc_has_option(ip6, ND_OPT_SOURCE_LL_ADDR)) {
