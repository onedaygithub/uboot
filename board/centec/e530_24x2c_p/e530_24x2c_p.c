...
void e530_24x_p_i2c_cfg(void)
{
	int i2cbus = 0;
	cmd_i2c_set_bus_num(i2cbus);

	/* Lcm cfg i2c to gpio for 24x2c-p*/
#if 0	
	ctc_i2c_mw(0x21, 0x6, 0x0);
	ctc_i2c_mw(0x21, 0x7, 0x0);
	ctc_i2c_mw(0x21, 0x2, 0x0);
	ctc_i2c_mw(0x21, 0x3, 0x0);

	ctc_i2c_mw(0x23, 0x6, 0x0);
	ctc_i2c_mw(0x23, 0x2, 0x0);
#else
	/* tx enable */
	ctc_i2c_mw(0x21, 0x2, 0x0);
	ctc_i2c_mw(0x21, 0x3, 0x0);
	...
	ctc_i2c_mw(0x22, 0x2, 0xbf);
	ctc_i2c_mw(0x22, 0x3, 0xff);
	/* bank 1 : input, bank 0 : output*/
	ctc_i2c_mw(0x22, 0x6, 0x0c);
	ctc_i2c_mw(0x22, 0x7, 0xff);

	/* tx enable */
	ctc_i2c_mw(0x23, 0x2, 0x0);
	ctc_i2c_mw(0x23, 0x3, 0xff);
	/* bank 1 : input, bank 0 : output*/
	ctc_i2c_mw(0x23, 0x6, 0x0);
	ctc_i2c_mw(0x23, 0x7, 0xff);
#endif
}
