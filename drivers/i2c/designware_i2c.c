					   ...
					   unsigned int speed)
{
	...
	case IC_SPEED_MODE_STANDARD:
		...
		//printf("CTC_DEBUG, SS, hcnt is %d, lcnt is %d, cntl is 0x%x\n", hcnt, lcnt, cntl);
	...
	default:
		...
		//printf("CTC_DEBUG, FS, hcnt is %d, lcnt is %d, cntl is 0x%x\n", hcnt, lcnt, cntl);
...
static void __dw_i2c_init(struct i2c_regs *i2c_base, int speed, int slaveaddr)
{
	...
	writel((IC_CON_SD | IC_CON_SPD_SS | IC_CON_MM), &i2c_base->ic_con);
